...
import { CheckCircle, Mail, Home, Diamond, Loader2 } from 'lucide-react';
...
export default function PaymentSuccess() {
  ...
  useEffect(() => {
    const processPayment = async () => {
      // Obtener referencia de la URL o localStorage
              ...
              <>
                ...
                {result?.status === 'processed' && (
                  <p className="text-lg text-green-400 font-medium mb-6">
                    ¡{result.diamonds} diamantes asignados!
                  </p>
                )}
                {result?.status === 'already_processed' && (
                  <p className="text-lg text-green-400 font-medium mb-6">
                    Tu compra ya fue procesada anteriormente
                  </p>
                )}
                {!result?.diamonds && result?.status !== 'already_processed' && (
                  <p className="text-lg text-green-400 font-medium mb-6">
